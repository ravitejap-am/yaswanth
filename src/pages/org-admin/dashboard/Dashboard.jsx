...
import { BASE_DOC_API_URL, BASE_ORG_API_URL } from '../../../constants/Constant'
import {
    Box,
    FormControl,
    Grid,
...
function Dashboard() {
    ...
    const getOrganisationCount = async () => {
        try {
            const response = await axios.get(
                `${BASE_ORG_API_URL}/all?active=true`,
                {
                    headers: {
                        Authorization: `Bearer ${jwt}`,
                    },
    ...
    const fetchUsageSubscriptionDetails = async () => {
        try {
            ...
            console.log(
                'error in fetching usage subscription details-->',
                error
            )
        }
    }
